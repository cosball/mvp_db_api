...
module.exports = function (User) {
    ...
    User.beforeRemote('create', function (ctx, modelInstance, next) {
        ...
        var token = encryption.decrypt(ctx.args.data.token);
        if (!token) {
            return next(error);
        }

        var textParts = token.split(',');
        // console.log("*** " + ctx.args.data.email + "," + ctx.req.headers.authorization)
        // console.log("*** " + textParts[2] + "," + textParts[0])
        // console.log(Date.now() - textParts[1]);
        if (textParts[2] !== ctx.args.data.email || ctx.req.headers.authorization !== textParts[0] || Date.now() - textParts[1] > 60000) {
            return next(error);
        }
        ...
        ctx.args.data.roleType = 'User';
        ctx.args.data.creator = 'webadmin';
