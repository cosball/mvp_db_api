...
module.exports = function(SkinData) {
  ...
  console.log("Translog model =", Translog)
	...
	SkinData.dashboard = function(institutionId, cb) {
		...
		var dashboardData = {
			skindata: 0,   
			skindataPerType: { BTC: 0, ETH: 0, XRP: 0, LTC: 0, BCH: 0, others: 0 },  
			transChecked: 0,
			transCheckedPerType: { BTC: 0, ETH: 0, XRP: 0, LTC: 0, BCH: 0, others: 0 },  	
			skindataPerPeriod: { daily: 0, weekly: 0, monthly: 0 } 
		}
		...
		SkinData.count(where, function(err, skindata) { 
			if (err) {return cb(err)};
			console.log("SkinDatas = ", skindata)
			dashboardData.skindata = skindata;
			where.addressType = 'BTC';

			SkinData.count(where, function(err, btclists) { 
				if (err) {return cb(err)};

				dashboardData.skindataPerType.BTC = btclists;
				where.addressType = 'ETH';

				SkinData.count(where, function(err, ethlists) { 
					if (err) {return cb(err)};

					dashboardData.skindataPerType.ETH = ethlists;
					where.addressType = 'XRP';
					
					SkinData.count(where, function(err, xrplists) { 
						if (err) {return cb(err)};

						dashboardData.skindataPerType.XRP = xrplists;
						where.addressType = 'LTC';

						SkinData.count(where, function(err, ltclists) { 
							if (err) {return cb(err)};

							dashboardData.skindataPerType.LTC = ltclists;
							where.addressType = 'BCH';

							SkinData.count(where, function(err, bchlists) { 
								if (err) {return cb(err)};

								dashboardData.skindataPerType.BCH = bchlists;
								dashboardData.skindataPerType.others = dashboardData.skindata - dashboardData.skindataPerType.BTC - dashboardData.skindataPerType.ETH -
																		 dashboardData.skindataPerType.XRP - dashboardData.skindataPerType.LTC - dashboardData.skindataPerType.BCH;

								SkinData.count(where_day, function(err, daylists) { 
									if (err) {return cb(err)};
									dashboardData.skindataPerPeriod.daily = daylists;
									SkinData.count(where_week, function(err, weeklists) { 
										if (err) {return cb(err)};
										dashboardData.skindataPerPeriod.weekly = weeklists;
										SkinData.count(where_month, function(err, monthlists) { 
											if (err) {return cb(err)};
											dashboardData.skindataPerPeriod.monthly = monthlists;
											var Translog = app.models.transLog;
  											console.log("Translog model =", Translog)
